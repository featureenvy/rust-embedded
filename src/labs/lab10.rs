...
impl State for signaling_state::AllRed {
    fn next(&self, pedestrian: bool, west: bool, north: bool) -> &'static State {
        match (pedestrian, west, north) {
            (true, _, _) => &signaling_state::GO_PEDESTRIAN_REF,
            (_, true, _) => &signaling_state::GO_WEST_REF,
            (_, _, true) => &signaling_state::GO_NORTH_REF,
            (_, _, _) => &signaling_state::ALL_RED_REF
        }
    }
...
impl State for signaling_state::LastCallWest {
    fn next(&self, pedestrian: bool, _: bool, north: bool) -> &'static State {
        match (pedestrian, north) {
            (_, true) => &signaling_state::GO_NORTH_REF,
            (true, _) => &signaling_state::GO_PEDESTRIAN_REF,
            (_, _) => &signaling_state::ALL_RED_REF
        }
    }
...
impl State for signaling_state::GoNorth {
    fn next(&self, _: bool, _: bool, _: bool) -> &'static State {
        &signaling_state::LAST_CALL_NORTH_REF
    }
...
impl State for signaling_state::GoPedestrian {
    fn next(&self, _: bool, _: bool, _: bool) -> &'static State {
        &signaling_state::LAST_CALL_PEDESTRIAN_REF
    }
...
impl State for signaling_state::LastCallPedestrian {
    fn next(&self, _: bool, west: bool, north: bool) -> &'static State {
        match (west, north) {
            (true, _) => &signaling_state::GO_WEST_REF,
            (_, true) => &signaling_state::GO_NORTH_REF,
            (_, _) => &signaling_state::ALL_RED_REF
        }
    }
